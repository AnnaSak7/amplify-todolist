...
export const fetchTodoListAsync = createAsyncThunk(
  ...
  async () => {
    ...
    return response; // response = todoList from fetchTodoListApi
  }
);
...
export const todoSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchTodoListAsync.fulfilled, (state, action) => {
      state.todoList = action.payload; // payload = response from fetchTodoListAsync
    });
  },
});

export const { fetchTodoRealTime, updateTodoRealTime, deleteTodoRealTime } =
  todoSlice.actions;
